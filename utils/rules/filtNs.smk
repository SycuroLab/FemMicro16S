rule filterNsRaw:
    ...
    output:
        R1=config["output_dir"]+"/primer_status/filtN/{sample}" + config["forward_read_suffix"] + config["compression_suffix"],
        R2=config["output_dir"]+"/primer_status/filtN/{sample}" + config["reverse_read_suffix"] + config["compression_suffix"]
    conda:
        "dada2"
    script:
